...
def _contact_sheet(frames: list[Image.Image]) -> bytes:
    ...
    for frame in prepared:
        sheet.paste(frame, (left, 0), frame)
...
def _raster_contact_sheet(data: bytes) -> bytes:
    try:
        ...
        UnidentifiedImageError, OSError, EOFError, ValueError,
    ) as exc:
...
def _normalize_image(data: bytes, content_type: str | None) -> tuple[str, bytes]:
    ...
    animated = (
        ...
        or (media_type == "image/png" and b"acTL" in data)
        or (media_type == "image/webp" and b"ANIM" in data)
    )
...
def _video_contact_sheet(data: bytes) -> bytes:
    try:
        with tempfile.NamedTemporaryFile(suffix=".video") as source:
            ...
            match = re.search(
                r"Duration: ([0-9]+):([0-9]+):([0-9]+(?:\.[0-9]+)?)", header,
            )
            duration = 1.0
            if match is not None:
                hours, minutes, seconds = match.groups()
                duration = max(
                    0.1, int(hours) * 3600 + int(minutes) * 60 + float(seconds),
                )
            fps = min(8.0, _MAX_FRAMES / duration)
            rendered = subprocess.run(
                [
                    executable, "-v", "error", "-i", source.name,
                    "-vf", (
                        f"fps={fps:.6f},"
    ...
    try:
        with LottieAnimation.from_data(data.decode("utf-8")) as animation:
            ...
            return _contact_sheet([
                animation.render_pillow_frame(index)
                for index in _sample_indices(total)
            ])
    except _WorkerError:
        raise
