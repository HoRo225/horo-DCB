...
def _decode_mapping(payload: object) -> tuple[int, dict[str, dict[str, object]]]:
    ...
    for key, record in payload["threads"].items():
        ...
        if (
            ...
            or (version == 2 and parent_channel_id is not None and (
            ...
            or (version == 2 and ":thread:" in key and parent_channel_id == int(key.rsplit(":", 1)[1]))
        ):
            raise ValueError("invalid thread mapping")
        if version == 1:
...
class ThreadStore:
    ...
    def get_parent(self, key: str) -> int | None:
        record = self._threads.get(key)
        return record.get("parent_channel_id") if record is not None else None  # type: ignore[return-value]
    ...
    def set(self, key: str, thread_id: str, *, parent_channel_id: int | None = None) -> None:
        ...
        existing_parent = current.get("parent_channel_id")
        ...
        parent = parent_channel_id or current.get("parent_channel_id")
    ...
    def bind_parent(self, key: str, parent_channel_id: int) -> None:
        if type(parent_channel_id) is not int or parent_channel_id <= 0:
            raise ValueError("invalid thread parent")
        record = self._threads.get(key)
        if record is None or ":thread:" not in key or parent_channel_id == int(key.rsplit(":", 1)[1]):
    ...
    def matching(
        ...
        *,
        ...
        return [
            key for key, record in self._threads.items()
            if scope_matches(
