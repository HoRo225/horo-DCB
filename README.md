...
horo-DCB 是以 Python、`discord.py`、9Router 與 Docker Compose 組成的 Discord AI Bot。專案把 AI 對話、受限制的 Web Research、選用的頻道語意記憶、Discord 行事曆看板、臨時語音、Steam 限時免費通知，以及管理員控制台整合在同一個 Bot 內。
...
- **Steam 限時免費**：查詢目前 100% 折扣且原本付費的遊戲，可選擇定時公開通知。
- **`/控制台`**：管理員專用、ephemeral 的 Discord Components V2 介面；可查看狀態、重新同步臨時語音及手動查詢 Steam。
...
| Steam 自動通知 | View Channels、Send Messages；要讓 Bot 自動建立通知頻道時另需 Manage Channels |

專案不要求 Bot 取得 Administrator 權限。執行 `/控制台` 與 `/行事曆 綁定`、`/行事曆 解除綁定`、`/行事曆 重新整理` 的使用者必須是伺服器管理員；從看板或 AI 確認新增／修改活動的使用者必須具有 Manage Events 或 Administrator。
...
既有部署若沒有明確設定這些變數，Compose 與應用程式仍保留舊版預設行為。新部署應以 `.env.example` 的明確值為準。
...
`NINEROUTER_URL` 由 Compose 固定為內部服務網址 `http://9router:20128/v1`，新安裝不需要自行設定。`NINEROUTER_IMAGE_SEARCH_PROVIDER` 可留空並回退到一般 Search Provider；功能旗標缺少或留空時使用向後相容預設。Embedding model／dimensions 若明確存在但留空或格式不合法，`check-env.sh`／preflight 會拒絕。
...
綁定、解除綁定與管理員手動刷新命令只有 Guild Administrator 可用：
