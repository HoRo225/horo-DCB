...
from src.admin.panel import MAX_STEAM_OFFERS_SHOWN, AdminPanelView
from src.ai.access import CodexAccess
from src.ai.protocol import CodexRuntimeStatus
from src.steam.notifier import SteamFetchResult, SteamGuildStatus, SteamOffer
...
class AdminPanelViewTest(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.codex_access = configured_access(True, 10, channel_ids=(20,))
    ...
    def test_overview_shows_health_setup_and_steam_tracking_without_detail(self):
        self.assertTrue(self.view.has_components_v2())
        text = self.text(self.view)
        self.assertIn("# 管理控制台\n-# 系統狀態與管理工具", text)
        self.assertIn("## 系統狀態\n**全部正常**", text)
        self.assertIn("臨時語音與 Steam 免費遊戲皆可用", text)
    ...
    def test_overview_reports_disabled_unconfigured_and_corrupt_ai_access(self):
        corrupt = configured_access(True, 10, channel_ids=(20,))
        corrupt.state_available = False
        cases = (
            (
                "disabled",
                configured_access(False, 10, channel_ids=(20,)),
                "**AI 助手 · 停用**",
                "AI 對話目前依設定停用",
            ),
            (
                "other_guild",
                configured_access(True, 11, channel_ids=(20,)),
                "**AI 助手 · 停用**",
                "此伺服器不在 AI 白名單",
            ),
            (
            ...
            (
                "missing_roles",
                configured_access(True, 10, channel_ids=(20,), role_ids=()),
                "**AI 助手 · 待設定**",
                "白名單身分組尚未設定",
            ),
            (
    ...
    async def test_overview_update_time_changes_only_on_overview_refresh(self):
        ...
        with patch("src.admin.panel.time.time", return_value=initial + 60):
    ...
    def test_ai_page_requires_roles_before_allowing_current_operator(self):
        self.view.codex_access = configured_access(role_ids=())
        self.view._render_ai()
        ...
        text = self.text(self.view)
        self.assertIn("請選擇白名單身分組；目前尚未開放", text)
        self.assertIn("目前操作者：未允許", text)
    ...
    async def test_role_change_archives_before_applying_new_roles(self):
        ...
        async def archive(guild_id):
            ...
            self.assertFalse(
                self.codex_access.allows(10, 20, frozenset({70}))
            )
    ...
    async def test_adding_channel_does_not_archive_existing_conversations(self):
        ...
        self.assertTrue(self.codex_access.allows(10, 20, frozenset({70})))
        self.assertTrue(self.codex_access.allows(10, 21, frozenset({70})))
        self.codex_client.archive_scope.assert_not_awaited()
    ...
    async def test_removing_channel_archives_guild_while_access_is_suspended(self):
        ...
        async def archive(guild_id):
            self.assertEqual(guild_id, 10)
            self.assertFalse(self.codex_access.allows(10, 21, frozenset({70})))
        ...
        self.assertTrue(self.codex_access.allows(10, 21, frozenset({70})))
        self.assertFalse(self.codex_access.allows(10, 20, frozenset({70})))
        self.codex_client.archive_scope.assert_awaited_once_with(10)
        self.assertEqual(interaction.response.defer_count, 1)
        self.assertEqual(len(interaction.original_edits), 1)
