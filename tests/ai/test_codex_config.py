...
class CodexConfigTest(unittest.TestCase):
    ...
    def test_enabled_codex_ignores_retired_environment_seeds(self):
        with patch.dict(
            "src.config.os.environ",
            {
                "DISCORD_TOKEN": "configured",
    ...
    def test_enabled_codex_needs_only_guild_and_token(self):
        with patch.dict(
            "src.config.os.environ",
            {
                "DISCORD_TOKEN": "configured",
        ...
        for users in ("303,303", "0303", "+303"):
            with self.subTest(users=users):
                with patch.dict(
                    ...
                    self.assertTrue(AppConfig.from_env().codex_enabled)
