...
class CodexAccessTest(unittest.TestCase):
    ...
    def test_allowlist_accepts_multiple_channels_with_exact_guild_and_role(self):
        ...
        self.assertTrue(access.allows(10, 20, frozenset({70})))
        self.assertTrue(access.allows(10, 21, frozenset({70})))
        self.assertFalse(access.allows(11, 20, frozenset({70})))
        self.assertFalse(access.allows(10, 22, frozenset({70})))
        self.assertFalse(access.allows(10, 20))
        self.assertFalse(configured_access(False, 10, channel_ids=(20,)).allows(10, 20, frozenset({70})))

    def test_selected_channels_persist_and_override_previous_selection(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(access.allows(10, 21, frozenset({70})))
            self.assertTrue(access.allows(10, 22, frozenset({70})))
            self.assertEqual(
                json.loads(path.read_text(encoding="utf-8")),
                {"version": 3, "guild_id": 10, "channel_ids": [21, 22], "role_ids": [70]},
            )
        ...
        self.assertTrue(restarted.allows(10, 22, frozenset({70})))
        self.assertFalse(restarted.allows(10, 99, frozenset({70})))

    def test_version_one_state_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "codex_access.json"
            path.write_text(
        ...
        self.assertFalse(access.allows(10, 20, frozenset({70})))
        self.assertFalse(access.allows(10, 99, frozenset({70})))

    def test_version_two_state_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "codex_access.json"
            path.write_text(
        ...
        self.assertEqual(access.role_ids, frozenset())
        self.assertFalse(access.state_available)
        self.assertFalse(access.allows(10, 20, frozenset()))
        self.assertFalse(access.allows(10, 20, frozenset({70})))
    ...
    def test_corrupt_channel_state_requires_channels_then_roles_for_recovery(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertFalse(access.allows(10, 22, frozenset({70})))
            self.assertFalse(access.allows(10, 23))
            access.set_roles(10, frozenset({70}))
            self.assertTrue(access.allows(10, 22, frozenset({70})))
    ...
    def test_boolean_guild_id_in_state_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "codex_access.json"
            path.write_text(
                json.dumps({"version": 3, "guild_id": True, "channel_ids": [20], "role_ids": [70]}),
                encoding="utf-8",
            )

            access = CodexAccess(True, 1, state_path=path)

        self.assertFalse(access.state_available)
        self.assertFalse(access.allows(1, 20))

    def test_boolean_version_in_state_fails_closed(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "codex_access.json"
            path.write_text(
                json.dumps({"version": True, "guild_id": 10, "channel_ids": [20], "role_ids": [70]}),
                encoding="utf-8",
            )
    ...
    def test_roles_persist_version_three_and_authorize_members(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            path = Path(temp_dir) / "codex_access.json"
            access = CodexAccess(True, 10, state_path=path)
            ...
            self.assertFalse(access.allows(10, 20, frozenset()))
            self.assertTrue(access.allows(10, 20, frozenset({60, 80})))
            self.assertFalse(access.allows(10, 20, frozenset({60})))
            self.assertEqual(
                json.loads(path.read_text(encoding="utf-8")),
                {
                    "version": 3,
        ...
        self.assertFalse(restarted.allows(10, 20, frozenset()))
        self.assertTrue(restarted.allows(10, 20, frozenset({70})))
    ...
    def test_codex_home_is_required_and_fixed(self):
        for value in ("", "/tmp"):
            with self.subTest(value=value):
                with patch.dict("src.ai.bridge.os.environ", {"CODEX_HOME": value}, clear=True):
                    with self.assertRaisesRegex(RuntimeError, "CODEX_HOME"):
                        _codex_home()

    def test_base_instructions_require_private_bounded_utf8_regular_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            path = root / "base_instructions.txt"
            path.write_text("test-only instructions", encoding="utf-8")
            path.chmod(0o600)
            self.assertEqual(_base_instructions(root), "test-only instructions")

            path.chmod(0o644)
            with self.assertRaises(RuntimeError):
                _base_instructions(root)
    ...
    async def test_new_conversation_persists_then_resumes_same_thread(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            service = CodexService(
                codex,
                store,
                base_instructions="test-only instructions",
                timeout_seconds=1,
            )
            key = "guild:1:channel:2:user:3"

            first = await service.chat(key, "one", ())
            second = await service.chat(key, "two", ())

        self.assertEqual((first, second), ("first", "second"))
        self.assertEqual(store.get(key), "thread-1")
        self.assertEqual(len(codex.started), 1)
        self.assertEqual(
            codex.started[0]["base_instructions"],
            "test-only instructions",
        )
        self.assertEqual(
            [inputs[0].text for inputs in codex.threads["thread-1"].calls],
            ["one", "two"],
        )
    ...
    async def test_same_conversation_serializes_while_different_ones_run_in_parallel(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            first = asyncio.create_task(
                same_service.chat("guild:1:thread:2", "one", ())
            )
            await same_codex.ready.wait()
            second = asyncio.create_task(
                same_service.chat("guild:1:thread:2", "two", ())
            )
...
class FakeBridgeService:
    ...
    async def chat(self, key, text, images):
        if self.error is not None:
            raise self.error
        self.chat_calls.append((key, text, images))
        return "answer"
...
class CodexBridgeClientStatusTest(unittest.IsolatedAsyncioTestCase):
    async def test_status_uses_three_second_timeout_separate_from_chat_timeout(self):
        ...
        reply = await client.chat("guild:1:thread:2", "hello", ())
...
class BridgeHttpTest(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_health_status_chat_and_archive_contract(self):
        ...
        self.assertEqual(
            ...
            [("guild:1:channel:2:user:3", "hello", ())],
        )
