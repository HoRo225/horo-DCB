...
- Semantic Memory、臨時語音、Steam 自動通知、AI TextDisplay 功能旗標。
...
前三項具有資料或 Discord 管理副作用，因此新安裝採 opt-in。既有部署缺少變數時，Compose／程式內預設仍維持舊行為，避免升級時偷偷關閉既有功能。
...
- 所有 AI 與 Steam 路徑使用 `AllowedMentions.none()`。
...
自動通知關閉時，手動 `fetch_current_offers()` 仍供控制台與 Agent 使用，且不產生公開訊息。

## 14. Discord 行事曆

### 14.1 Source of truth 與綁定狀態
...
`/行事曆 綁定`、`解除綁定`、`重新整理` 只允許 Guild Administrator。綁定前先驗證 Bot 在目標文字頻道具有 View Channel、Send Messages，以及 Guild 層級 Create Events、Manage Events；Calendar 看板不再依賴 Read Message History，因為已知 `message_id` 的維護直接使用 `PartialMessage`。成功建立新看板並持久化新 binding 後，才 best-effort 刪除舊看板。解除綁定則先移除並持久化 binding，再刪除舊訊息，因此舊按鈕立即失效，即使訊息刪除失敗也不能再操作。
...
| `admin_panel.py` | 管理 UI 與公開模組動作 | 第二份持久化狀態 |
| `bot.py` | Discord adapter、事件路由、composition root | Provider 協定細節 |
